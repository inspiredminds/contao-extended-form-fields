<?php

declare(strict_types=1);

/*
 * This file is part of the ContaoExtendedFormFields bundle.
 *
 * (c) inspiredminds
 *
 * @license LGPL-3.0-or-later
 */

$GLOBALS['TL_LANG']['tl_form_field']['minOptions'] = ['Minimale Anzahl', 'Die minimale Anzahl an Optionen, die ausgewählt werden müssen.'];
$GLOBALS['TL_LANG']['tl_form_field']['maxOptions'] = ['Maximale Anzahl', 'Die maximale Anzahl an Optionen, die ausgewählt werden können.'];
$GLOBALS['TL_LANG']['tl_form_field']['errorMsg'] = ['Eigene Fehlermeldung', 'Diese Fehlermeldung wird bei einer ungültigen Eingabe angezeigt.'];
$GLOBALS['TL_LANG']['tl_form_field']['addCustomOption'] = ['Eigene Angabe erlauben', 'Fügt ein Textfeld hinzu, womit der Benutzer einen eigenen Wert eingeben kann.'];
$GLOBALS['TL_LANG']['tl_form_field']['disallowedValues'] = ['Nicht erlaubte Wörter', 'Diese Wörter verursachen einen Fehler bei der Validierung.'];
$GLOBALS['TL_LANG']['tl_form_field']['allowedValues'] = ['Erlaubte Werte', 'Nur diese Wörter bzw. Werte werden erlaubt.'];
$GLOBALS['TL_LANG']['tl_form_field']['httpurl'] = ['Absolute URL', 'Überprüft ob die eingegebene URL eine absolute URL ist.'];
$GLOBALS['TL_LANG']['tl_form_field']['showCharacterCount'] = ['Zeichen-Anzahl zeigen', 'Zeigt die eingegebene Anzahl an Zeichen über JavaScript.'];
$GLOBALS['TL_LANG']['tl_form_field']['defaultFromRequest'] = ['Standard-Wert aus Anfrage', 'Setzt den Standard-Wert dieses Feldes anhand eines Query-Parameters des selben Namens.'];
